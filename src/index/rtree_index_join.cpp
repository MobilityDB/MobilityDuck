/* A bounding-box join answered by probing a TRTREE index once per row of the
 * other side.
 *
 ...
 */
#include "index/rtree_index_join.hpp"
...
void LogicalRTreeIndexJoin::ResolveTypes() {
...
vector<ColumnBinding> LogicalRTreeIndexJoin::GetColumnBindings() {
...
PhysicalOperator &LogicalRTreeIndexJoin::CreatePlan(ClientContext &context, PhysicalPlanGenerator &planner) {
	auto &child = planner.CreatePlan(*children[0]);
	auto &op = planner.Make<PhysicalRTreeIndexJoin>(types, index_table, index, probe_column, probe_is_temporal,
	                                                index_columns, estimated_cardinality);
	auto &join = op.Cast<PhysicalRTreeIndexJoin>();
...
PhysicalRTreeIndexJoin::PhysicalRTreeIndexJoin(PhysicalPlan &physical_plan, vector<LogicalType> types_p,
                                               DuckTableEntry &index_table_p, TRTreeIndex &index_p,
...
class RTreeIndexJoinState : public OperatorState {
public:
...
unique_ptr<OperatorState> PhysicalRTreeIndexJoin::GetOperatorState(ExecutionContext &context) const {
	auto state = make_uniq<RTreeIndexJoinState>();
...
OperatorResultType PhysicalRTreeIndexJoin::Execute(ExecutionContext &context, DataChunk &input, DataChunk &chunk,
                                                   GlobalOperatorState &, OperatorState &state_p) const {
	auto &state = state_p.Cast<RTreeIndexJoinState>();
	...
	while (state.probe_row < input.size()) {
		...
		if (state.match_offset == 0 && state.matches.empty()) {
			 ...
			 * rows it holds (rtree_module.cpp): a Temporal column is indexed by
...
//===--------------------------------------------------------------------===//
// Optimizer: recognise `a.col && b.col` over a table carrying a TRTREE index
//===--------------------------------------------------------------------===//

class TRTreeIndexJoinOptimizer : public OptimizerExtension {
public:
	TRTreeIndexJoinOptimizer() {
...
private:
	...
	//! (rtree_module.cpp:100-107). Everything else is refused: `tspatial_to_stbox`
	...
	static bool TryBuild(ClientContext &context, unique_ptr<LogicalOperator> &plan, LogicalAnyJoin &join,
		...
		auto result = make_uniq<LogicalRTreeIndexJoin>(table, *found, probe_ref.binding.column_index,
...
void TRTreeModule::RegisterJoinOptimizer(ExtensionLoader &loader) {
	loader.GetDatabaseInstance().config.optimizer_extensions.push_back(TRTreeIndexJoinOptimizer());
}
