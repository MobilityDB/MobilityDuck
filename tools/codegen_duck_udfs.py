...
RETIRED_GROUPS = {"meos_temporal_analytics_similarity", "meos_temporal_comp_temp",
                  ...
                  # Static QUADBIN cell-index scalar ops (getResolution/cellToParent/
                  # cellSibling/tileToCell/cellArea/cellToQuadkey/isValidCell/isValidIndex)
                  # over the BIGINT-backed quadbin() type — generated by shape_quadbin_scalar;
                  # the hand quadbinGetResolution/quadbinTileToCell/quadbinIsValidCell regs are
                  # deleted in the same wave. The geometry/Set/array cell ops stay uncovered
                  # (RETIRE_UNCOVERED_OK) pending the MEOS export relay (P1–P4).
                  "meos_quadbin"}
...
# @sqlfn of a RETIRED group the generator does NOT emit yet — kept out of the retire-safety
# trap-2 with a documented reason (each needs a MEOS export or a marshalling shape not yet built).
RETIRE_UNCOVERED_OK = {
    # meos_quadbin cell ops blocked on a MEOS-C export gap (relayed to the MEOS lane, mirror h3):
    "geoToQuadbinCell",        # SQL (geometry,int); MEOS lacks quadbin_gs_point_to_cell (P1)
    "quadbinCellToChildren",   # uint64* array; MEOS should return Set* like h3 (P2)
    "quadbinCellToPoint",      # quadbin_cell_to_geompoint(uint64)->GSERIALIZED* (MEOS #1482);
                               # needs a Duck cell->geometry shape (still arg:uint64)
    "quadbinCellToBoundary",   # quadbin_cell_to_geom(uint64)->GSERIALIZED* (MEOS #1482); same shape gap
    "quadbinCellToBoundingBox",# now @sqlfn=None (stbox(quadbin) cast replacement in flight); kept harmless
    "quadbinCellToTile",       # void + 3 out-params → int[]; needs an array/tuple export (P4)
    "quadbin_out",             # type output function — hand I/O / cast layer, never generated
    "stbox",                   # stbox(quadbin)/stbox(quadbin,timestamptz) casts+constructors (the
                               # stbox-cast MEOS PR that replaced quadbinCellToBoundingBox); cast
                               # layer, stays hand like the quadbin ↔VARCHAR casts
}
def retired(f):
...
def shape_quadbin_scalar(f):
    """The static QUADBIN cell-index scalar ops of the @ingroup meos_quadbin group: they take
    and/or return the raw uint64 cell id — the BIGINT-backed quadbin() DuckDB type — alongside
    small int / double / text operands, all VALUE types the generic arg_type/ret_type do not
    marshal. Keyed on the C-signature shape (a sibling of shape_h3_prefilter). NOT matched here
    (deferred to RETIRE_UNCOVERED_OK, need a MEOS export or extra marshalling): geoToQuadbinCell
    (SQL (geometry,int) — MEOS lacks the GSERIALIZED-taking export, relay P1), cellToChildren
    (uint64* array, P2), cellToPoint/cellToBoundingBox (void + out-params → geometry, P3),
    cellToTile (void + 3 out-params → int[], P4), quadbin_out (type I/O, cast layer)."""
    if (f.get("group") or "") != "meos_quadbin":
        return None
    if not f.get("sqlfn"):
        return None
    # The type IN/OUT layer (quadbin_out <- quadbin_index_to_string) is classified 'io-cast' by
    # supported() and is served by the hand quadbin<->VARCHAR cast — never a generated scalar.
    if (supported(f) or "").split(":")[0] == "io-cast":
        return None
    ins, out = classify(f)
    if out is not None:
        return None
    cs = [norm(p["canonical"]) for p in ins]
    rn = norm(f["returnType"]["canonical"])
    U64, UINT, TXT = "uint64_t", "unsigned int", "char *"   # norm() strips the leading const
    if cs == [U64]:
        if rn == "bool":                                          return "cell2bool"
        if rn in ("unsigned int", "uint32_t", "int", "int32_t"):  return "cell2int"
        if rn == "double":                                        return "cell2double"
        if rn == TXT:                                             return "cell2text"
        return None
    if cs == [U64, UINT] and rn == U64:   return "cellint2cell"   # quadbin_cell_to_parent
    if cs == [U64, TXT] and rn == U64:    return "cellstr2cell"   # quadbin_cell_sibling
    if cs == [UINT, UINT, UINT] and rn == U64:  return "tile2cell"  # quadbin_tile_to_cell
    return None

def emit_quadbin_scalar(f, kind):
    """Emit the Gen_ body for a static QUADBIN scalar op. quadbin() is BIGINT-backed, so a uint64
    cell id rides in an int64_t (bit-preserving; quadbin cell ids clear the sign bit), matching
    the cellscalar temporal-accessor idiom. A char* return is an owned MEOS string → TakeCString
    marshals + frees it."""
    name = f["name"]
    def unary(rett_cpp, body):
        return (f"static void Gen_{name}(DataChunk &args, ExpressionState &, Vector &result) {{\n"
                f"    EnsureMeosThreadInitialized();\n"
                f"    UnaryExecutor::Execute<int64_t, {rett_cpp}>(args.data[0], result, args.size(),\n"
                f"        [&](int64_t cell) -> {rett_cpp} {{\n"
                f"            {body}\n"
                f"        }});\n}}\n")
    if kind == "cell2bool":
        return unary("bool",    f"return {name}((uint64_t) cell);")
    if kind == "cell2int":
        return unary("int32_t", f"return (int32_t) {name}((uint64_t) cell);")
    if kind == "cell2double":
        return unary("double",  f"return {name}((uint64_t) cell);")
    if kind == "cell2text":
        return unary("string_t", f"char *r = {name}((uint64_t) cell);\n"
                                 f"            return TakeCString(result, r);")
    if kind == "cellint2cell":
        return (f"static void Gen_{name}(DataChunk &args, ExpressionState &, Vector &result) {{\n"
                f"    EnsureMeosThreadInitialized();\n"
                f"    BinaryExecutor::Execute<int64_t, int32_t, int64_t>(args.data[0], args.data[1], result, args.size(),\n"
                f"        [&](int64_t cell, int32_t k) -> int64_t {{\n"
                f"            return (int64_t) {name}((uint64_t) cell, (uint32_t) k);\n"
                f"        }});\n}}\n")
    if kind == "cellstr2cell":
        return (f"static void Gen_{name}(DataChunk &args, ExpressionState &, Vector &result) {{\n"
                f"    EnsureMeosThreadInitialized();\n"
                f"    BinaryExecutor::Execute<int64_t, string_t, int64_t>(args.data[0], args.data[1], result, args.size(),\n"
                f"        [&](int64_t cell, string_t dir) -> int64_t {{\n"
                f"            return (int64_t) {name}((uint64_t) cell, dir.GetString().c_str());\n"
                f"        }});\n}}\n")
    # tile2cell: (int, int, int) -> quadbin
    return (f"static void Gen_{name}(DataChunk &args, ExpressionState &, Vector &result) {{\n"
            f"    EnsureMeosThreadInitialized();\n"
            f"    TernaryExecutor::Execute<int32_t, int32_t, int32_t, int64_t>(args.data[0], args.data[1], args.data[2], result, args.size(),\n"
            f"        [&](int32_t x, int32_t y, int32_t z) -> int64_t {{\n"
            f"            return (int64_t) {name}((uint32_t) x, (uint32_t) y, (uint32_t) z);\n"
            f"        }});\n}}\n")
...
def gen_cpp(fns, out_path, declared=None, aliases=None):
    ...
    # ---- QUADBIN static cell-index scalar ops (@ingroup meos_quadbin; sibling of the H3
    # prefilter above): get-resolution / cell-to-parent / cell-sibling / tile-to-cell / area /
    # quadkey / isValid over the BIGINT-backed quadbin() type. GATED on retired(f): a bare
    # generated name double-registers with a live hand reg → SIGSEGV, so only emit once the
    # group's hand regs are being deleted (meos_quadbin ∈ RETIRED_GROUPS). ----
    for f in fns:
        if not retired(f):
            continue
        qpk = shape_quadbin_scalar(f)
        if qpk is None:
            continue
        STATE["grp"] = f.get("group") or "meos_ungrouped"
        set_bodies.append(emit_quadbin_scalar(f, qpk)); n_set += 1
        fn = f["name"]
        names = reg_names(f, f["sqlfn"], aliases)
        Q = "QuadbinTypes::quadbin()"
        if qpk == "cell2bool":
            sig, rett = "{" + Q + "}", "LogicalType::BOOLEAN"
        elif qpk == "cell2int":
            sig, rett = "{" + Q + "}", "LogicalType::INTEGER"
        elif qpk == "cell2double":
            sig, rett = "{" + Q + "}", "LogicalType::DOUBLE"
        elif qpk == "cell2text":
            sig, rett = "{" + Q + "}", "LogicalType::VARCHAR"
        elif qpk == "cellint2cell":
            sig, rett = "{" + Q + ", LogicalType::INTEGER}", Q
        elif qpk == "cellstr2cell":
            sig, rett = "{" + Q + ", LogicalType::VARCHAR}", Q
        else:   # tile2cell
            sig, rett = "{LogicalType::INTEGER, LogicalType::INTEGER, LogicalType::INTEGER}", Q
        for nm in names:
            set_specific_regs.append(f'    RegisterSerializedScalarFunction(loader, ScalarFunction('
                                     f'"{reg_name(nm, f)}", {sig}, {rett}, Gen_{fn}));')
