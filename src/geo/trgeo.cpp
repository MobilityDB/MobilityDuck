...
extern "C" {
    ...
    // meos_rgeo.h, so this port routes through the subtype-agnostic
    // spatial parser tspatial_parse (a real exported symbol that
    // trgeo_in itself delegates to with T_TRGEOMETRY), exactly the way
    // the canonical MobilityDB SQL binds trgeometryFromText /
    // trgeometry_in. Declared locally because the internal
    // geo/tspatial_parser.h is not part of the installed MEOS headers.
...
LogicalType TRGeoTypes::TRGEOMETRY() {
    auto type = LogicalType(LogicalTypeId::BLOB);
    type.SetAlias("TRGEOMETRY");
    return type;
}

static inline Temporal *Trgeo_parse_text(const std::string &str) {
    const char *ptr = str.c_str();
    return tspatial_parse(&ptr, T_TRGEOMETRY);
}
...
inline void Trgeo_constructor(DataChunk &args, ExpressionState &state, Vector &result) {
    ...
    UnaryExecutor::Execute<string_t, string_t>(
        ...
        [&](string_t input_geom_str) -> string_t {
            ...
            Temporal *tinst = Trgeo_parse_text(input);
            if (!tinst) {
...
TInstant **temparr_extract_rg(Vector &trgeo_arr_vec, list_entry_t list_entry, int *count) {
    auto &child_vector = ListVector::GetEntry(trgeo_arr_vec);
...
inline void Trgeo_sequence_constructor(DataChunk &args, ExpressionState &state, Vector &result) {
    // Default values
    ...
    auto &trgeo_arr_vec = args.data[0];
    trgeo_arr_vec.Flatten(count);
    ...
    auto trgeo_data = FlatVector::GetData<list_entry_t>(trgeo_arr_vec);
    auto result_data = FlatVector::GetData<string_t>(result);

    // Get validity masks
    auto &trgeo_validity = FlatVector::Validity(trgeo_arr_vec);
    auto &result_validity = FlatVector::Validity(result);

    for (idx_t i = 0; i < count; i++) {
        if (!trgeo_validity.RowIsValid(i)) {
            result_validity.SetInvalid(i);
            continue;
        }

        try {
            list_entry_t list_entry = trgeo_data[i];
            ...
            int element_count;
            TInstant **instants = temparr_extract_rg(trgeo_arr_vec, list_entry, &element_count);
...
// The trgeometry value is a moving rigid geometry: a reference geometry
// rotated/translated over time by an embedded temporal pose. It is not a
// registered DuckDB type. geom(trgeometry) returns the reference geometry
// via trgeo_geom; startValue / endValue return the geometry snapshot at
// the first / last instant via trgeo_start_value / trgeo_end_value (both
// header-declared, returning a freshly allocated GSERIALIZED the caller
// owns), surfaced as a GEOMETRY blob, mirroring how the tpose port
// surfaces its value accessors.

inline void Trgeo_geom(DataChunk &args, ExpressionState &state, Vector &result) {
    auto count = args.size();
    ...
    UnaryExecutor::Execute<string_t, string_t>(
        ...
        [&](string_t input_str) -> string_t {
            ...
            // trgeo_geom returns a freshly allocated GSERIALIZED copy of
            // the reference geometry, which the caller owns.
            GSERIALIZED *gs = trgeo_geom(temp);
            if (!gs) {
...
inline void Temporal_start_value(DataChunk &args, ExpressionState &state, Vector &result) {
    ...
    UnaryExecutor::Execute<string_t, string_t>(
        ...
        [&](string_t input_str) -> string_t {
            ...
            // trgeo_start_value returns a freshly allocated GSERIALIZED
            // copy of the rigid-geometry snapshot at the first instant.
            GSERIALIZED *gs = trgeo_start_value(temp);
            if (!gs) {
...
inline void Temporal_end_value(DataChunk &args, ExpressionState &state, Vector &result) {
    ...
    UnaryExecutor::Execute<string_t, string_t>(
        ...
        [&](string_t input_str) -> string_t {
            ...
            // trgeo_end_value returns a freshly allocated GSERIALIZED
            // copy of the rigid-geometry snapshot at the last instant.
            GSERIALIZED *gs = trgeo_end_value(temp);
            if (!gs) {
...
void TRGeoTypes::RegisterScalarFunctions(ExtensionLoader &loader) {

    auto trgeo_function = ScalarFunction(
        "TRGEOMETRY",
        {LogicalType::VARCHAR},
        TRGeoTypes::TRGEOMETRY(),
        Trgeo_constructor
    );
    duckdb::RegisterSerializedScalarFunction(loader,  trgeo_function);

    auto trgeoseqarr_1param= ScalarFunction(
        "trgeometrySeq",
        {LogicalType::LIST(TRGeoTypes::TRGEOMETRY())},
        TRGeoTypes::TRGEOMETRY(),
        Trgeo_sequence_constructor
    );
    duckdb::RegisterSerializedScalarFunction(loader,  trgeoseqarr_1param);

    auto trgeoseqarr_2params = ScalarFunction(
        "trgeometrySeq",
        {LogicalType::LIST(TRGeoTypes::TRGEOMETRY()), LogicalType::VARCHAR},
        TRGeoTypes::TRGEOMETRY(),
        Trgeo_sequence_constructor
    );
    duckdb::RegisterSerializedScalarFunction(loader,  trgeoseqarr_2params);

    auto trgeoseqarr_3params = ScalarFunction(
        "trgeometrySeq",
        {LogicalType::LIST(TRGeoTypes::TRGEOMETRY()), LogicalType::VARCHAR, LogicalType::BOOLEAN},
        TRGeoTypes::TRGEOMETRY(),
        Trgeo_sequence_constructor
    );
    duckdb::RegisterSerializedScalarFunction(loader,  trgeoseqarr_3params);

    auto trgeoseqarr_4params = ScalarFunction(
        "trgeometrySeq",
        {LogicalType::LIST(TRGeoTypes::TRGEOMETRY()), LogicalType::VARCHAR, LogicalType::BOOLEAN, LogicalType::BOOLEAN},
        TRGeoTypes::TRGEOMETRY(),
        Trgeo_sequence_constructor
    );
    duckdb::RegisterSerializedScalarFunction(loader,  trgeoseqarr_4params);

    auto trgeo_to_timespan_function = ScalarFunction(
        "timeSpan",
        {TRGeoTypes::TRGEOMETRY()},
        SpanTypes::TSTZSPAN(),
        Temporal_to_tstzspan);
    duckdb::RegisterSerializedScalarFunction(loader,  trgeo_to_timespan_function);

    auto trgeo_to_tinstant_function = ScalarFunction(
        "trgeometryInst",
        {TRGeoTypes::TRGEOMETRY()},
        TRGeoTypes::TRGEOMETRY(),
        Temporal_to_tinstant);
    duckdb::RegisterSerializedScalarFunction(loader,  trgeo_to_tinstant_function);


    auto setInterp_function = ScalarFunction(
        "setInterp",
        {TRGeoTypes::TRGEOMETRY(), LogicalType::VARCHAR},
        TRGeoTypes::TRGEOMETRY(),
        Temporal_set_interp
    );
    ...
    auto merge_function = ScalarFunction(
        "merge",
        {TRGeoTypes::TRGEOMETRY(), TRGeoTypes::TRGEOMETRY()},
        TRGeoTypes::TRGEOMETRY(),
        Temporal_merge
    );
    ...
    auto trgeo_geom_function = ScalarFunction(
        "geom",
        {TRGeoTypes::TRGEOMETRY()},
        GeoTypes::GEOMETRY(),
        Trgeo_geom
    );
    duckdb::RegisterSerializedScalarFunction(loader,  trgeo_geom_function);

    auto trgeo_start_value_function = ScalarFunction(
        "startValue",
        {TRGeoTypes::TRGEOMETRY()},
        GeoTypes::GEOMETRY(),
        Temporal_start_value
    );
    duckdb::RegisterSerializedScalarFunction(loader,  trgeo_start_value_function);

    auto trgeo_end_value_function = ScalarFunction(
        "endValue",
        {TRGeoTypes::TRGEOMETRY()},
        GeoTypes::GEOMETRY(),
        Temporal_end_value
    );
    duckdb::RegisterSerializedScalarFunction(loader,  trgeo_end_value_function);
    ...
    auto instantN_function = ScalarFunction(
        "instantN",
        {TRGeoTypes::TRGEOMETRY(), LogicalType::INTEGER},
        TRGeoTypes::TRGEOMETRY(),
        Temporal_instant_n
    );
    ...
    auto trgeo_gettimestamptz_function = ScalarFunction(
        "getTimestamp",
        {TRGeoTypes::TRGEOMETRY()},
        LogicalType::TIMESTAMP_TZ,
        Tinstant_timestamptz);
    duckdb::RegisterSerializedScalarFunction(loader,  trgeo_gettimestamptz_function);


    // ===================================================================
    // Foundational trgeo surface — accessors, time/value-restrict,
    ...
    const LogicalType TGEOM = TRGeoTypes::TRGEOMETRY();
...
void TRGeoTypes::RegisterTypes(ExtensionLoader &loader) {
    loader.RegisterType( "TRGEOMETRY", TRGeoTypes::TRGEOMETRY());
}
