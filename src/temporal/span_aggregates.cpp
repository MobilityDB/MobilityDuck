...
// ============================================================================
// setUnion(set) -> set
// State is a Set* accumulated via set_union_transfn.
// ============================================================================

namespace {

struct SetUnionState {
    Set *set;
};
...
struct SetUnionFunction {
    ...
    static void Operation(STATE &state, const INPUT_TYPE &input, AggregateUnaryInput &) {
        ...
        state.set = set_union_transfn(state.set, s);
        free(s);
    }
    ...
    template <class STATE, class OP>
    static void Combine(const STATE &source, STATE &target, AggregateInputData &) {
        if (!source.set || source.set->count == 0) return;
        const_cast<STATE &>(target).set = set_union_transfn(target.set, source.set);
    }

    template <class T, class STATE>
    static void Finalize(STATE &state, T &target, AggregateFinalizeData &finalize_data) {
        if (!state.set) { finalize_data.ReturnNull(); return; }
        Set *result = set_union_finalfn(state.set);
        state.set = nullptr;
        if (!result) { finalize_data.ReturnNull(); return; }
        size_t out_size = set_mem_size(result);
        target = finalize_data.ReturnString(
            string_t(reinterpret_cast<const char *>(result), out_size));
        free(result);
    }
    ...
    template <class STATE>
    static void Destroy(STATE &state, AggregateInputData &) {
        if (state.set) { free(state.set); state.set = nullptr; }
    }
};

static AggregateFunction MakeSetUnionAggregate(const LogicalType &set_type) {
    return AggregateFunction::UnaryAggregateDestructor<SetUnionState, string_t, string_t, SetUnionFunction>(
        set_type, set_type);
}

} // namespace

void SpanAggregates::RegisterSetUnion(ExtensionLoader &loader) {
    AggregateFunctionSet setunion_set("setUnion");
    for (const auto &set_type : SetTypes::AllTypes()) {
        setunion_set.AddFunction(MakeSetUnionAggregate(set_type));
    }
    loader.RegisterFunction(std::move(setunion_set));
}
