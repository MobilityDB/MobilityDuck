...
--
-- The temporal value stays an opaque MEOS-WKB BLOB (the ratified spec).
-- To let a reader skip Parquet row groups WITHOUT decoding any row, also
-- project O(1) native-scalar bound columns from the trajectory's inline
-- spatiotemporal box (stbox): plain TIMESTAMPTZ / DOUBLE columns carry real
-- Parquet min/max statistics, so engine predicate pushdown prunes on them.
-- These sidecars are additive ordinary columns; the footer still describes
-- only the WKB column, so the file stays spec-conformant.
-- ─────────────────────────────────────────────────────────────────────────────
...
COPY (
    SELECT
        ...
        Tmin(stbox(traj)) AS ts_min,   -- native time-bound sidecars
        Tmax(stbox(traj)) AS ts_max,
        Xmin(stbox(traj)) AS lon_min,  -- native space-bound sidecars
        Xmax(stbox(traj)) AS lon_max,
        Ymin(stbox(traj)) AS lat_min,
        Ymax(stbox(traj)) AS lat_max
    FROM trajectories
)
...
-- Verify the Parquet schema: traj is an opaque BYTE_ARRAY; the sidecars
-- are native scalar columns (the prunable shape)
SELECT name, type
...
-- ─────────────────────────────────────────────────────────────────────────────
-- Query D: the lake predicate-pushdown pattern
--
-- Filter on the native-scalar sidecars FIRST (plain TIMESTAMPTZ / DOUBLE
-- comparisons).  DuckDB reads each Parquet row group's min/max statistics
-- and skips groups that cannot match, WITHOUT decoding any MEOS-WKB byte.
-- tgeogpointFromBinary() then runs only on the surviving rows.  On a real
-- lake (millions of rows, remote object storage) this is the difference
-- between scanning the whole dataset and touching a few row groups.
--
-- Window: vessels active in the Øresund box during the first hour.
-- ─────────────────────────────────────────────────────────────────────────────
SELECT
    entity_id,
    round(length(tgeogpointFromBinary(traj))) AS length_m
FROM read_parquet('edge_to_cloud_demo.parquet')
WHERE ts_min >= TIMESTAMPTZ '2026-01-15 08:00:00+00'
  AND ts_max <  TIMESTAMPTZ '2026-01-15 09:00:00+00'
  AND lon_min <= 13.5 AND lon_max >= 11.5
  AND lat_min <= 56.5 AND lat_max >= 55.0
ORDER BY entity_id;

-- To see the pruning in the plan:
--   EXPLAIN ANALYZE SELECT ... (the query above)
-- and look for the Parquet scan's pushed Filters and reduced row count.
