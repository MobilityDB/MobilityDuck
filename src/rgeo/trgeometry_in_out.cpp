...
// meos_rgeo.h is intentionally not included (its trgeo_append_tinstant
...
extern "C" {
    ...
    // reference geometry as WKT). trgeo_out (above) emits the HexWKB geom
...
// includes the upstream one-line fix (trgeo_in must call trgeo_parse).
...
inline void Trgeometry_as_text_exec(DataChunk &args, ExpressionState &state, Vector &result) {
    ...
    UnaryExecutor::Execute<string_t, string_t>(
        ...
        [&](string_t input_geom_str) -> string_t {
            ...
            // (trgeo_out emits the HexWKB geom form instead).
...
// `trgeo_in` and `trgeometry_from_mfjson` are the renamed,
