...
LogicalType TNpointTypes::TNPOINT() {
...
static void Tnpoint_constructor(DataChunk &args, ExpressionState &state, Vector &result) {
    ...
    UnaryExecutor::Execute<string_t, string_t>(
        ...
        [&](string_t input_geom_str) -> string_t {
            ...
            Temporal *tinst = tnpoint_in(input.c_str());
            if (!tinst) {
                throw InvalidInputException("Invalid TNPOINT input: " + input);
            }
...
static void Tnpointinst_constructor(DataChunk &args, ExpressionState &state, Vector &result) {
    auto count = args.size();
    ...
    BinaryExecutor::Execute<string_t, timestamp_tz_t, string_t>(
        ...
        [&](string_t value_str, timestamp_tz_t t) -> string_t {
            ...
            // The tnpoint value type is an Npoint (route id + relative
            // position), a network-relative reference with no intrinsic
            // coordinates, parsed from its canonical text form, e.g.
            // 'Npoint(1, 0.5)'.
            Npoint *np = npoint_in(value.c_str());

            if (np == NULL) {
                throw InvalidInputException("Invalid network point format: " + value);
            }

            timestamp_tz_t meos_timestamp = DuckDBToMeosTimestamp(t);
            // No tnpointinst_make exists; the generic tinstant_make
            // builds a T_TNPOINT instant from the Npoint Datum.
            TInstant *inst = tinstant_make(Datum(np), T_TNPOINT,
            ...
            if (!data_buffer){
                free(inst);
                free(np);
                throw InvalidInputException("Failed to allocate memory to network point data");
            }
...
static void Tnpoint_sequence_from_tstzspan(DataChunk &args, ExpressionState &state, Vector &result) {
    ...
    BinaryExecutor::Execute<string_t, string_t, string_t>(
        ...
        [&](string_t input_geom_str, string_t span_str)-> string_t{
            ...
            Npoint *np = npoint_in(geom_value.c_str());

            if(np == NULL){
                throw InvalidInputException("Invalid network point format: "+ geom_value);
            }
            ...
            TSequence *seq = tsequence_from_base_tstzspan(Datum(np), T_TNPOINT, span_cmp, interp);
...
TInstant **temparr_extract_np(Vector &tnpoint_arr_vec, list_entry_t list_entry, int *count) {
    auto &child_vector = ListVector::GetEntry(tnpoint_arr_vec);
...
static void Tnpoint_sequence_constructor(DataChunk &args, ExpressionState &state, Vector &result) {
    // Default values
    const char* default_interp = "linear";
    ...
    auto &tnpoint_arr_vec = args.data[0];
    tnpoint_arr_vec.Flatten(count);
    ...
    auto tnpoint_data = FlatVector::GetData<list_entry_t>(tnpoint_arr_vec);
    auto result_data = FlatVector::GetData<string_t>(result);

    // Get validity masks
    auto &tnpoint_validity = FlatVector::Validity(tnpoint_arr_vec);
    auto &result_validity = FlatVector::Validity(result);

    for (idx_t i = 0; i < count; i++) {
        if (!tnpoint_validity.RowIsValid(i)) {
            result_validity.SetInvalid(i);
            continue;
        }

        try {
            list_entry_t list_entry = tnpoint_data[i];
            ...
            int element_count;
            TInstant **instants = temparr_extract_np(tnpoint_arr_vec, list_entry, &element_count);
...
static void Temporal_to_tstzspan(DataChunk &args, ExpressionState &state, Vector &result) {
    ...
    UnaryExecutor::Execute<string_t, string_t>(
        ...
        [&](string_t input_str) -> string_t {
            ...
            if (!timespan) {
                throw InvalidInputException("Failed to extract timespan from TNPOINT");
            }
...
static void Temporal_to_tinstant(DataChunk &args, ExpressionState &state, Vector &result) {
    ...
    UnaryExecutor::Execute<string_t, string_t>(
        ...
        [&](string_t input_str) -> string_t {
            ...
            if (!inst) {
                throw InvalidInputException("Failed to convert TNPOINT to TInstant");
            }
...
static void Temporal_merge(DataChunk &args, ExpressionState &state, Vector &result) {
    ...
    BinaryExecutor::Execute<string_t, string_t, string_t>(
        ...
        [&](string_t tgeom1_str_t, string_t tgeom2_str_t) -> string_t {
            ...
            if (!result_temp) {
                throw InvalidInputException("Failed to merge temporal network points");
            }
...
// ---- Network-point value accessors ----
// The tnpoint value type is an Npoint (route id + relative position),
// which is not a registered DuckDB type and is network-relative with no
// intrinsic coordinates. getValue / startValue / endValue surface it in
// its canonical text form (`NPoint(rid,pos)`), mirroring the asText
// output. route(tnpoint) exposes the route identifier per the manual
// model via the subtype-agnostic tnpoint_route MEOS function.
//
// NOTE: the value executors carry tnpoint-unique names (Tnpoint_get_value /
// Tnpoint_start_value / Tnpoint_end_value). Generic names (Tinstant_value /
// Temporal_start_value / Temporal_end_value) are also defined at namespace
// scope in the other geo .cpp; that is an ODR violation and the linker would
// keep a single (geometry) definition for all of them, silently rendering the
// npoint as a geometry ("Unknown geometry type: 0").

inline Npoint *npoint_from_instant_value(const TInstant *inst) {
    Datum d = tinstant_value(inst);
    return reinterpret_cast<Npoint*>(d);
}

static void Tnpoint_get_value(DataChunk &args, ExpressionState &state, Vector &result) {
    auto count = args.size();
    ...
    UnaryExecutor::Execute<string_t, string_t>(
        ...
        [&](string_t input_str) -> string_t {
            ...
            // Npoint value (datum_copy), which the caller owns.
            Npoint *np = npoint_from_instant_value(tinst);

            char *str = npoint_out(np, 15);
            if (!str) {
                free(np);
                throw InvalidInputException("Failed to convert network point value to text");
            }
...
static void Tnpoint_start_value(DataChunk &args, ExpressionState &state, Vector &result) {
    auto count = args.size();
    auto &input_vec = args.data[0];
    ...
    UnaryExecutor::Execute<string_t, string_t>(
        ...
        [&](string_t input_str) -> string_t {
            ...
            // the Npoint value (datum_copy), which the caller owns.
            Datum start_datum = temporal_start_value(temp);

            Npoint *np = reinterpret_cast<Npoint*>(start_datum);
            char *str = npoint_out(np, 15);
            if (!str) {
                free(np);
                throw InvalidInputException("Failed to convert network point value to text");
...
static void Tnpoint_end_value(DataChunk &args, ExpressionState &state, Vector &result) {
    auto count = args.size();
    auto &input_vec = args.data[0];
    ...
    UnaryExecutor::Execute<string_t, string_t>(
        ...
        [&](string_t input_str) -> string_t {
            ...
            Npoint *np = reinterpret_cast<Npoint*>(end_datum);
            char *str = npoint_out(np, 15);
            if (!str) {
                free(np);
                throw InvalidInputException("Failed to convert network point value to text");
...
static void Tnpoint_route(DataChunk &args, ExpressionState &state, Vector &result) {
    auto count = args.size();
    auto &input_vec = args.data[0];
    ...
    UnaryExecutor::Execute<string_t, int64_t>(
        ...
        [&](string_t input_str) -> int64_t {
            ...
            // tnpoint_route is subtype-agnostic; it returns the single
            // route identifier the temporal network point traverses.
            int64 rid = tnpoint_route(temp);
            return static_cast<int64_t>(rid);
        });
...
void TNpointTypes::RegisterScalarFunctions(ExtensionLoader &loader) {

    auto tnpoint_function = ScalarFunction(
        "TNPOINT",
        {LogicalType::VARCHAR},
        TNpointTypes::TNPOINT(),
        Tnpoint_constructor
    );
    duckdb::RegisterSerializedScalarFunction(loader,  tnpoint_function);

    auto tnpoint_from_timestamp_function = ScalarFunction(
        "TNPOINT",
        {LogicalType::VARCHAR, LogicalType::TIMESTAMP_TZ},
        TNpointTypes::TNPOINT(),
        Tnpointinst_constructor);
    duckdb::RegisterSerializedScalarFunction(loader,  tnpoint_from_timestamp_function);

     auto tnpoint_from_tstzspan_function = ScalarFunction(
        "TNPOINT",
        {LogicalType::VARCHAR, SpanTypes::TSTZSPAN(), LogicalType::VARCHAR},
        TNpointTypes::TNPOINT(),
        Tnpoint_sequence_from_tstzspan
    );
    duckdb::RegisterSerializedScalarFunction(loader,  tnpoint_from_tstzspan_function);

    auto tnpoint_from_tstzspan_default = ScalarFunction(
        "TNPOINT",
        {LogicalType::VARCHAR, SpanTypes::TSTZSPAN()},
        TNpointTypes::TNPOINT(),
        Tnpoint_sequence_from_tstzspan
    );
    duckdb::RegisterSerializedScalarFunction(loader,  tnpoint_from_tstzspan_default);

     auto tnpointseqarr_1param= ScalarFunction(
        "tnpointSeq",
        {LogicalType::LIST(TNpointTypes::TNPOINT())},
        TNpointTypes::TNPOINT(),
        Tnpoint_sequence_constructor
    );
    duckdb::RegisterSerializedScalarFunction(loader,  tnpointseqarr_1param);

    auto tnpointseqarr_2params = ScalarFunction(
        "tnpointSeq",
        {LogicalType::LIST(TNpointTypes::TNPOINT()), LogicalType::VARCHAR},
        TNpointTypes::TNPOINT(),
        Tnpoint_sequence_constructor
    );
    duckdb::RegisterSerializedScalarFunction(loader,  tnpointseqarr_2params);

    auto tnpointseqarr_3params = ScalarFunction(
        "tnpointSeq",
        {LogicalType::LIST(TNpointTypes::TNPOINT()), LogicalType::VARCHAR, LogicalType::BOOLEAN},
        TNpointTypes::TNPOINT(),
        Tnpoint_sequence_constructor
    );
    duckdb::RegisterSerializedScalarFunction(loader,  tnpointseqarr_3params);

    auto tnpointseqarr_4params = ScalarFunction(
        "tnpointSeq",
        {LogicalType::LIST(TNpointTypes::TNPOINT()), LogicalType::VARCHAR, LogicalType::BOOLEAN, LogicalType::BOOLEAN},
        TNpointTypes::TNPOINT(),
        Tnpoint_sequence_constructor
    );
    duckdb::RegisterSerializedScalarFunction(loader,  tnpointseqarr_4params);

    auto tnpoint_to_timespan_function = ScalarFunction(
        "timeSpan",
        {TNpointTypes::TNPOINT()},
        SpanTypes::TSTZSPAN(),
        Temporal_to_tstzspan);
    duckdb::RegisterSerializedScalarFunction(loader,  tnpoint_to_timespan_function);

    auto tnpoint_to_tinstant_function = ScalarFunction(
        "tnpointInst",
        {TNpointTypes::TNPOINT()},
        TNpointTypes::TNPOINT(),
        Temporal_to_tinstant);
    duckdb::RegisterSerializedScalarFunction(loader,  tnpoint_to_tinstant_function);


    auto setInterp_function = ScalarFunction(
        "setInterp",
        {TNpointTypes::TNPOINT(), LogicalType::VARCHAR},
        TNpointTypes::TNPOINT(),
        Temporal_set_interp
    );
    ...
    auto merge_function = ScalarFunction(
        "merge",
        {TNpointTypes::TNPOINT(), TNpointTypes::TNPOINT()},
        TNpointTypes::TNPOINT(),
        Temporal_merge
    );
    ...
    auto tnpoint_start_value_function = ScalarFunction(
        "startValue",
        {TNpointTypes::TNPOINT()},
        LogicalType::VARCHAR,
        Tnpoint_start_value
    );
    duckdb::RegisterSerializedScalarFunction(loader,  tnpoint_start_value_function);

    auto tnpoint_end_value_function = ScalarFunction(
        "endValue",
        {TNpointTypes::TNPOINT()},
        LogicalType::VARCHAR,
        Tnpoint_end_value
    );
    duckdb::RegisterSerializedScalarFunction(loader,  tnpoint_end_value_function);

    auto tnpoint_route_function = ScalarFunction(
        "route",
        {TNpointTypes::TNPOINT()},
        LogicalType::BIGINT,
        Tnpoint_route
    );
    duckdb::RegisterSerializedScalarFunction(loader,  tnpoint_route_function);
    ...
    auto instantN_function = ScalarFunction(
        "instantN",
        {TNpointTypes::TNPOINT(), LogicalType::INTEGER},
        TNpointTypes::TNPOINT(),
        Temporal_instant_n
    );
    ...
    auto tnpoint_gettimestamptz_function = ScalarFunction(
        "getTimestamp",
        {TNpointTypes::TNPOINT()},
        LogicalType::TIMESTAMP_TZ,
        Tinstant_timestamptz);
    duckdb::RegisterSerializedScalarFunction(loader,  tnpoint_gettimestamptz_function);


    // ===================================================================
    // Foundational tnpoint surface — accessors, time/value-restrict,
    ...
    const LogicalType TGEOM = TNpointTypes::TNPOINT();
...
void TNpointTypes::RegisterTypes(ExtensionLoader &loader) {
    loader.RegisterType( "TNPOINT", TNpointTypes::TNPOINT());
}
