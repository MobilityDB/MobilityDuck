...
--
-- The native-scalar bound columns (projected O(1) from each trajectory's
-- inline stbox) are additive ordinary TIMESTAMPTZ / DOUBLE columns: they
-- carry real Parquet min/max statistics so a reader prunes row groups on a
-- time/space predicate without decoding any MEOS-WKB byte.  The footer
-- still describes only the WKB column, so the file stays spec-conformant.
-- ─────────────────────────────────────────────────────────────────────────────
...
COPY (
    SELECT
        ...
        Tmin(stbox(traj)) AS ts_min,
        Tmax(stbox(traj)) AS ts_max,
        Xmin(stbox(traj)) AS lon_min,
