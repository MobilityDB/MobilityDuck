 ...
 *
 * Implementation pattern: Bind extracts the input tgeompoint blob and the
 * scalar tile parameters; Init calls the matching MEOS export
 * (tgeo_space_split / tgeo_space_time_split), buffers the parallel result
 * arrays as Value triples; Exec drains the buffer into the output chunk.
 * Same shape as the existing TemporalUnnest TableFunction in
