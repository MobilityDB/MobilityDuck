...
template <double (*FN)(const STBox *, const GSERIALIZED *)>
void StboxGeoNadExec(DataChunk &args, ExpressionState &, Vector &result) {
    BinaryExecutor::Execute<string_t, string_t, double>(
        args.data[0], args.data[1], result, args.size(),
        [&](string_t b_blob, string_t g_blob) -> double {
            STBox *b = DecodeStboxCopy(b_blob);
            int32 srid = stbox_srid(b);
            GSERIALIZED *gs = GeometryToGSerialized(g_blob, srid);
            double r = FN(b, gs);
            free(b); free(gs);
            return r;
...
template <double (*FN)(const STBox *, const GSERIALIZED *)>
void GeoStboxNadExec(DataChunk &args, ExpressionState &, Vector &result) {
    BinaryExecutor::Execute<string_t, string_t, double>(
        args.data[0], args.data[1], result, args.size(),
        [&](string_t g_blob, string_t b_blob) -> double {
            STBox *b = DecodeStboxCopy(b_blob);
            int32 srid = stbox_srid(b);
            GSERIALIZED *gs = GeometryToGSerialized(g_blob, srid);
            double r = FN(b, gs);
...
template <double (*FN)(const STBox *, const STBox *)>
void StboxStboxNadExec(DataChunk &args, ExpressionState &, Vector &result) {
    BinaryExecutor::Execute<string_t, string_t, double>(
        args.data[0], args.data[1], result, args.size(),
        [&](string_t a, string_t b) -> double {
            STBox *b1 = DecodeStboxCopy(a);
            STBox *b2 = DecodeStboxCopy(b);
            double r = FN(b1, b2);
            free(b1); free(b2);
            return r;
        });
...
#undef TREL_REG
    ...
    // tCovers — extra `restr/atvalue` config args MobilityDB hides
    // behind defaults; register the simplest shape (no restriction).
    ...
    // eCovers — int-returning ever-covers, fits the existing
    // GeoTgeoIntExec / TgeoGeoIntExec / TgeoTgeoIntExec shape.
    ...
    // counterparts to `<->`/`tdistance`. The eight-way NAD overload set
    // mirrors MobilityDB's 064_tgeo_distance.in.sql.
    // -----------------------------------------------------------------
    ...
    loader.RegisterFunction(ScalarFunction("nearestApproachDistance",
        {STBOX, GEOM}, DBL, StboxGeoNadExec<nad_stbox_geo>));
    loader.RegisterFunction(ScalarFunction("nearestApproachDistance",
        {GEOM, STBOX}, DBL, GeoStboxNadExec<nad_stbox_geo>));
    loader.RegisterFunction(ScalarFunction("nearestApproachDistance",
        {STBOX, STBOX}, DBL, StboxStboxNadExec<nad_stbox_stbox>));
    ...
    // The tgeompoint signatures dispatch to the same subtype-agnostic
    // MEOS C functions; the registrations exist purely to make the
    // overload resolver pick them up without an explicit cast.
    const LogicalType TGP = TgeompointType::TGEOMPOINT();

    loader.RegisterFunction(ScalarFunction("nearestApproachDistance",
        {TGP, GEOM}, DBL, TgeoGeoNadExec<nad_tgeo_geo>));
    loader.RegisterFunction(ScalarFunction("nearestApproachDistance",
        {GEOM, TGP}, DBL, GeoTgeoNadExec<nad_tgeo_geo>));
    loader.RegisterFunction(ScalarFunction("nearestApproachDistance",
        {TGP, TGP}, DBL, TgeoTgeoNadExec<nad_tgeo_tgeo>));
    loader.RegisterFunction(ScalarFunction("nearestApproachDistance",
        {TGP, STBOX}, DBL, TgeoStboxNadExec<nad_tgeo_stbox>));
    loader.RegisterFunction(ScalarFunction("nearestApproachDistance",
        {STBOX, TGP}, DBL, StboxTgeoNadExec<nad_tgeo_stbox>));
    ...
    loader.RegisterFunction(ScalarFunction("nearestApproachInstant",
        {TGP, GEOM}, TGP, TgeoGeoNaiExec<nai_tgeo_geo>));
    loader.RegisterFunction(ScalarFunction("nearestApproachInstant",
        {TGP, TGP}, TGP, TgeoTgeoNaiExec<nai_tgeo_tgeo>));

    loader.RegisterFunction(ScalarFunction("shortestLine",
        {TGP, GEOM}, GEOM, TgeoGeoShortestLineExec<shortestline_tgeo_geo>));
    // (TGP, TGP) shortestLine already registered in tgeompoint.cpp.
    ...
    // splitNStboxes / splitEachNStboxes — partition a tspatial value
    // into N stboxes (or N elements per box). Subtype-agnostic MEOS
    // call — same exec works for tgeometry, tgeompoint, tgeography,
    // tgeogpoint. The latter two are registered in tgeography_ops.cpp.
    ...
    // Same registrations for tgeompoint — MEOS dispatches by subtype.
    loader.RegisterFunction(ScalarFunction(
        "splitNStboxes", {TgeompointType::TGEOMPOINT(), LogicalType::INTEGER},
