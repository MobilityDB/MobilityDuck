...
void StboxFunctions::Stbox_from_hexwkb(DataChunk &args, ExpressionState &state, Vector &result) {
    UnaryExecutor::Execute<string_t, string_t>(
        ...
        [&](string_t input_hexwkb) -> string_t {
            // string_t::GetData() is not NUL-terminated; stbox_from_hexwkb()
            // strlen()s its argument, so pass a NUL-terminated copy to avoid
            // reading past the buffer (matches the sibling hex-WKB consumers).
