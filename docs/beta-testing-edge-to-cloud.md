...
3. The trajectories are written to a **TemporalParquet** shard: a standard
   ...
   type, encoding, and CRS. The writer additionally projects O(1)
   native-scalar bound columns (`ts_min`/`ts_max` TIMESTAMPTZ,
   `lon_min`/`lon_max`/`lat_min`/`lat_max` DOUBLE) from each trajectory's
   inline `stbox`. These are additive ordinary columns — the footer still
   describes only the WKB column, so the file stays spec-conformant — but
   they carry real Parquet min/max statistics, so a time/space predicate
   prunes row groups without decoding any MEOS-WKB byte.
