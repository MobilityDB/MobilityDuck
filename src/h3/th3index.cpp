...
void H3IndexFunctions::Ever_eq_h3indexset_th3index(DataChunk &args,
...
void H3IndexFunctions::Ever_eq_th3index_h3indexset(DataChunk &args,
    ExpressionState &state, Vector &result) {
    BinaryExecutor::ExecuteWithNulls<string_t, string_t, bool>(
        args.data[0], args.data[1], result, args.size(),
        [&](string_t temp_blob, string_t set_blob, ValidityMask &mask,
            idx_t idx) -> bool {
            Temporal *t = BlobToTemp(temp_blob);
            Set *s = BlobToSet(set_blob);
            int r = ever_eq_h3indexset_th3index(s, t);
...
void H3IndexTypes::RegisterScalarFunctions(ExtensionLoader &loader) {
    ...
    duckdb::RegisterSerializedScalarFunction(loader, ScalarFunction(
        "everEq", {H3SET, TH3}, LogicalType::BOOLEAN,
        H3IndexFunctions::Ever_eq_h3indexset_th3index));
    duckdb::RegisterSerializedScalarFunction(loader, ScalarFunction(
        "everEq", {TH3, H3SET}, LogicalType::BOOLEAN,
        H3IndexFunctions::Ever_eq_th3index_h3indexset));
}
